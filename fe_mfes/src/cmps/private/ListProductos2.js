...
const ListProductos2 = () =>{
    ...
    const listElements =  prods.productos.map((o)=>{
        ...
        return (<li key={o._id} >{o.nombre} 
         <img className="product" src={jeans}/>
         <div className="flex">{subdocument.map((ob)=>
         <span style={{background : ob.cod_color}} key={ob.ide} onClick={() => { dispatch({ type: PRODUCT_SET_CURRENT, payload:{_id:o._id}}); history.push("/productos/one2");}}>
    ...
    return (
        <Page headding="Jeans" footer={true}>
            ...
            onClick={(e)=>{history.push('/productos/new2')}}>

            </AddButton>
        </Page>
    );
}
export default ListProductos2;